...
class AudiosControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create audio" do
    assert_difference('Audio.count') do
      post audios_url, params: { audio: { file_name: @audio.file_name, status: @audio.status, title: @audio.title, user_id: @audio.user_id } }
    end
  ...
  test "should update audio" do
    patch audio_url(@audio), params: { audio: { file_name: @audio.file_name, status: @audio.status, title: @audio.title, user_id: @audio.user_id } }
